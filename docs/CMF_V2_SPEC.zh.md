...
CMF 是工程容器：共享骨干可以与任务掩码和覆盖张量组合，而无需复制或
改写骨干。格式记录经过测量的质量元数据，并明确保存行与输入通道的
缩放等实现细节，让读取器能够校验每个分区。
...
| 2   | `QUANT_2F`     | 目录包含 `q8_2f`/`vbit` 张量（多尺度量化） |
...
- **`q8_2f`**：`[int8][f16 row-scale][f16 col-scale]`，
  `w = q·scale[o]·col[i]` —— 独立的行缩放与输入通道缩放，已在
  vmfcore 中验证（同尺寸下 +37%；在离群输入通道上恢复了 q8→f16 差距的约 75%）。
...
任务掩码 = 在共享权重之上标记“哪些活跃”的位字段。掩码为任务选择
行与注意力头，共享权重本身不变。
...
- **未经测量的声明** —— 默认不写入 `quality_score: 1.0`、容量等主张。
  只有定义了指标并记录测量结果，字段才会进入格式。
