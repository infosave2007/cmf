...
отгейтить по ppl до физического выброса. А если нужен ОДИН файл с
переключаемыми специалистами — `cortiq moe-mask … --name coder`
запекает ограничение именованной задачной маской вместо среза:
активация `run --task coder`, токен-в-токен с физическим срезом
(без выигрыша в размере и пейджинге, но ничего не выброшено). Вне
задачи качество падает по построению — кодовая маска покрывает лишь
~39% прозовой массы роутинга.
