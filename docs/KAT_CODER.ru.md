...
Проверка: `cortiq --version` → 0.5.27 или новее.
...
## 5. Вырезать кодер-специалиста (опционально, обе платформы)

Использование экспертов в MoE сильно задачно — у KAT код и проза
маршрутизируются в почти непересекающиеся наборы (Jaccard top-64 =
0.25). Если модель будет обслуживать ОДНУ задачу, она возит сотни
экспертов, к которым никогда не обращается. Убираются они двумя
командами:

```sh
# 1. Записать, каких экспертов задача реально использует (любой
#    репрезентативный корпус; здесь: исходники кодовой базы, ~5 мин на M4)
CMF_MOE_STATS=code_stats.json cortiq ppl kat-q4t.cmf --file my_code.txt --tokens 3072

# 2. Выбросить всё за пределами 95% записанной массы роутинга
cortiq moe-defrag kat-q4t.cmf --stats code_stats.json --cover 0.95 --output kat-coder.cmf
```

Замерено (кодовая калибровка, скоринг на held-out коде, M4 24 ГБ):
**19.6 → 12.7 ГБ (−35%)** при +2.8% перплексии — и поскольку 12.7 ГБ
влезают в память машины, где полная модель пейджилась, декод растёт
**7.6 → 13.7 tok/s (×1.8)**, префилл **6.1 → 20.0 (×3.3)**. Специалист —
обычный `.cmf`: все команды выше работают с ним без изменений. Проверить
уровень cover без перезаписи файла:
`CMF_MOE_MASK=code_stats.json CMF_MOE_MASK_COVER=0.95 cortiq ppl …` —
то же ограничение в рантайме (математика идентична), можно
отгейтить по ppl до физического выброса. Вне задачи качество падает по
построению — кодовая маска покрывает лишь ~39% прозовой массы роутинга.

## 6. Опциональные рычаги (обе платформы)
