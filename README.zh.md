...
把评价轴说清楚：`llama.cpp` 是我们对标的基准——就单流纯速度而言，今天它在前面。
一次同条件对比（2026-07-16：Qwen2.5-0.5B-Instruct，Apple M4，双方均为精确注意力，
原生 arm64 `llama.cpp` master 对 CMF 0.3.1，各 2+ 组独立进程）：
...
真实差距分解得很清楚：`llama.cpp` 的 decode 靠重排的交错 Q8 内核达到约 85% 的
内存带宽（我们约 53%——加载时行重排已排入计划），它的 CPU prefill 通过
Accelerate GEMM 跑在 Apple 的 AMX 单元上，它的 Metal 路径是完整的设备端计算图，
而 wgpu 后端还没有。CMF 的立足点不在直线加速赛：文件在对齐质量下小 26%，注意力
内存可以是 O(1)（`--o1` 在精确注意力从 15.7 掉到 8.2 tok/s 的上下文长度下稳在约
16.5），而且整个引擎是可移植的 Rust，无需 BLAS 或 C++ 工具链。一切都能用
`cortiq bench --json` 复现——它连同每词元分配数与调度派发数一起输出 tok/s。
