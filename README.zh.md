...
把评价轴说清楚：`llama.cpp` 是我们对标的基准——从 0.3.0 起，CPU 引擎在短提示单流
场景下也站得住了。一次完整的同条件对比（2026-07-16：Qwen2.5-0.5B-Instruct，Apple
Silicon，仅 CPU，双方各 8 线程，双方均为精确注意力，`llama.cpp` b9310 对 CMF
0.3.0，各 3+ 组独立进程）：

| CPU，8 线程 | `llama.cpp` (q8_0) | CMF (q8) | Δ |
|---|---|---|---|
| 提示处理 (pp512) | 220.6 ± 2.5 tok/s | 374–390 tok/s | **+70%** |
| 生成 (tg128) | 60.4 ± 1.1 tok/s | 95–101 tok/s | **+60%** |
| 量化质量（PPL 对各自 f16，12×512 窗口） | 近乎无损 | +0.38% | 已对齐 |
| 文件大小 | 644 MB | 479 MB | **−26%** |

一台机器上的一个模型只是一个数据点，不是排行榜：两种量化同属一类但并非逐字节相同
（8.06 对 8.5 比特/权重），完整的对比矩阵（模型 × 规模 × CPU、bootstrap 置信区间）
仍在路上。方法论随仓库同行——`cortiq bench --json` 会连同每词元的内存分配数与调度
派发数一起输出 tok/s——请在你自己的硬件上复现，而不是听我们说。在短场景之外，CMF
压平的是开销随上下文增长的曲线：随着上下文变长，精确注意力从 15.7 tok/s 衰减到
8.2，而 `--o1` 稳在约 16.5。这张表说的是短场景；长上下文那条尾巴，交给 `--o1`。
