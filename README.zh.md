...
（GeGLU、sandwich 归一化、512 滑动窗口 + 双 RoPE）· phi-3 / phi-4（拆分融合的
qkv/gate_up，longrope 按原生窗口提供）· DeepSeek-R1 蒸馏版（qwen2/llama 布局）
——涵盖 dense、MoE 和 GatedDeltaNet。尚不支持：gemma-2（注意力 softcapping）和
...
master 对 CMF 0.3.4，交替运行、各自独立进程，双方都取各自实测最优线程数——
...
| pp1024，CPU | — | 976 tok/s | 曲线不再塌陷（0.3.3 是 390） |
