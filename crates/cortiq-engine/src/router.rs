...
pub fn decide(
    ...
    let mut scores: Vec<SkillRoute> = idx
        ...
        .collect();
    if scores.is_empty() {
        return Routing { scores, confidence: 0.0, margin: 0.0, novelty: 1.0, is_novel: true, calibrated: calib.is_some() };
    }
    let (Some(c), Some(&top)) = (calib, idx.first()) else {
        let e_min = scores[0].error;
        return Routing { scores, confidence: 0.0, margin: 0.0, novelty: f32::NAN, is_novel: e_min > fallback_tau, calibrated: false };
    };
    // confidence: temperature softmax over −raw_error
    let mut logits: Vec<f32> = idx.iter().map(|&i| -rows[i].1 / c.temperature.max(1e-3)).collect();
    softmax(&mut logits);
    ...
    let margin = if idx.len() > 1 { inv(rows[idx[0]].1) - inv(rows[idx[1]].1) } else { inv(rows[idx[0]].1) };
    // energy: winner z-score against its training shell
    let (em, es) = (rows[top].2.unwrap_or(0.0), rows[top].3.unwrap_or(1.0).max(1e-4));
    let z = (rows[top].1 - em) / es;
    let novelty = NOVELTY_W_ENERGY * sigmoid(z) + NOVELTY_W_MARGIN / (1.0 + margin * NOVELTY_MARGIN_K) + NOVELTY_W_CONF * (1.0 - confidence);
    Routing { scores, confidence, margin, novelty, is_novel: novelty > c.novelty_theta, calibrated: true }
}

/// Per-skill error rows for a φ (skills with malformed descriptors skipped).
pub fn error_rows(model: &CmfModel, phi_of_layer: &mut dyn FnMut(usize) -> Vec<f32>) -> Vec<(String, f32, Option<f32>, Option<f32>, f32)> {
...
pub fn route_full(model: &CmfModel, pipeline: &mut Pipeline, ids: &[u32], fallback_tau: f32) -> Routing {
...
pub fn holdout_phis(model: &CmfModel) -> Vec<(usize, Vec<f32>)> {
    ...
    for (si, skill) in model.header.skills.iter().enumerate() {
        let Some(sel) = &skill.selection else { continue };
        let (Some(h), Some(n)) = (sel.holdout.as_ref(), sel.holdout_n) else { continue };
...
pub fn calibrate(model: &CmfModel, target_fpr: f32) -> Option<cortiq_core::format::RoutingCalibration> {
    ...
    let mut per_sample: Vec<(Vec<(String, f32, Option<f32>, Option<f32>, f32)>, usize)> = Vec::new();
    for (si, phi) in &samples {
        let layer = skills[*si].selection.as_ref().map(|s| s.phi_layer).unwrap_or(0);
        let mut phi_of = |l: usize| -> Vec<f32> { if l == layer { phi.clone() } else { Vec::new() } };
        let rows = error_rows(model, &mut phi_of);
        let Some(pos) = rows.iter().position(|r| r.0 == skills[*si].id) else { continue };
    ...
    let mut cal = cortiq_core::format::RoutingCalibration { temperature: best_t, novelty_theta: 0.5, samples: per_sample.len(), target_fpr };
    // θ: (1−fpr) quantile of the in-scope novelty scores
    let mut nov: Vec<f32> = per_sample.iter().map(|(rows, _)| decide(rows, Some(&cal), 1.0).novelty).filter(|v| v.is_finite()).collect();
