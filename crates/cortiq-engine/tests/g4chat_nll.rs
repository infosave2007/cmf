...
fn g4_chat_answer_nll() {
    ...
    let out = pl2.generate_from_ids(&prompt_ids, 40, None, None).unwrap();
    ...
    assert!(
        ok + 2 >= tot,
        "scorer argmax diverges from greedy decode: {ok}/{tot}"
    );
    let sg = (nll2 / cnt2.max(1) as f64).exp();
    assert!(
        sg < 3.0,
        "self-greedy ppl {sg:.2} — scorer disagrees with decode path"
    );
}
