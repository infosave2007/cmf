...
fn growth_appends_experts_without_touching_old_records() {
    ...
    let by0: std::collections::HashMap<&str, (usize, usize)> = lay
        .names
        .iter()
    ...
    for (name, o1, l1) in &lay1.names {
        if let Some((o0, l0)) = by0.get(name.as_str()) {
            assert_eq!(
                &ck.params[*o0..*o0 + l0],
                &grown.params[*o1..*o1 + l1],
                "{name} changed by growth"
            );
        }
    }
    ...
    for (name, o1, l1n) in &lay1.names {
        if let Some((o0, l0n)) = by0.get(name.as_str()) {
            assert_eq!(
                &ck.params[*o0..*o0 + l0n],
                &trained.params[*o1..*o1 + l1n],
                "{name} changed by training the new experts"
            );
        }
    }
    ...
    for t in &m0.tensors {
        ...
        assert_eq!(
            m0.tensor_bytes(&t.name).unwrap(),
            m1.tensor_bytes(&t.name).unwrap(),
            "{}: bytes changed",
            t.name
        );
        same += 1;
    ...
    eprintln!(
        "export: {same} old tensors byte-identical, {added} new records (experts.2.* + descriptors)"
    );
    assert!(
        m1.tensor("model.layers.0.mlp.experts.2.gate_proj.weight")
            .is_some()
    );
