...
fn runtime_matches_trainer_logprobs() {
    ...
    let tokens: Vec<u32> = lcg_vec(21, t).iter().map(|x| ((x * 0.5 + 0.5) * cfg.vocab as f32) as u32 % cfg.vocab as u32).collect();
    ...
    let ck = Checkpoint { cfg: cfg.clone(), step: 0, params: params.clone(), m: None, v: None, extras };
    ...
    let mut pipe = cortiq_engine::pipeline::Pipeline::from_model(&model, cortiq_engine::sampler::SamplerConfig::default()).expect("pipeline");
    ...
    for pos in [0usize, 1, 5, 17, 40, 63] {
        ...
        eprintln!("pos {pos}: max|Δ logprob| = {d:.3e}  (argmax trainer {} runtime {})",
            want.iter().enumerate().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).unwrap().0,
            got.iter().enumerate().max_by(|a, b| a.1.partial_cmp(b.1).unwrap()).unwrap().0);
        worst = worst.max(d);
    }
    let _ = std::fs::remove_dir_all(&dir);
    assert!(worst < 2e-3, "runtime vs trainer log-probs differ: max {worst}");
}
