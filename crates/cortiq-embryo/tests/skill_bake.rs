...
fn bake_appends_a_byte_identical_skill_the_runtime_loads() {
    ...
    assert!(
        lb <= l0 + 1e-3,
        "the skill must not be worse than the base on its own held-out ({lb} vs {l0})"
    );
    ...
    assert!(
        m1.tensor("skill.docs.model.layers.1.mlp.shared_expert.down_proj.weight")
            .is_some()
    );
    ...
    let mut p_skill =
        cortiq_engine::pipeline::Pipeline::from_model_with_skill(&m1, sc, Some("docs")).unwrap();
    let ids: Vec<u32> = corpus.tokens[1000..1032]
        .iter()
        .map(|&x| x as u32)
        .collect();
