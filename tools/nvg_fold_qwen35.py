...
def main():
    ...
    # ── hooks: Grams of the read-out inputs ──
    grams = {}
    def mk_hook(name):
        def h(mod, inp):
    ...
    with torch.no_grad():
        for si in range(calib.shape[0]):
            ...
            if si % 4 == 0:
                el = time.time() - t0
                log(f"calib seq {si+1}/{calib.shape[0]}  {el:.0f}s  ({el/(si+1):.1f}s/seq)")
    for hnd in handles: hnd.remove()
    log(f"calibration done in {time.time()-t0:.0f}s")
    ...
    # ── write: same schema as the source (model.language_model.*), sharded ──
    ...
    for k, v in new_sd.items():
        ...
        out_sd[k2] = v
    # shard by ~4 GB
    keys = list(out_sd.keys()); shard_i = 0; cur = {}; cur_b = 0; weight_map = {}
    def flush():
        nonlocal shard_i, cur, cur_b
        if not cur: return
        ...
        shard_i += 1; cur = {}; cur_b = 0
    for k in keys:
        v = out_sd[k]; b = v.numel() * v.element_size()
        if cur_b + b > 4e9 and cur: flush()
        cur[k] = v; cur_b += b
    flush()
    total = sum(v.numel() for v in out_sd.values())
    json.dump({"metadata": {"total_size": int(sum(v.numel()*v.element_size() for v in out_sd.values()))}, "weight_map": weight_map},
    ...
    for f in ["tokenizer.json", "tokenizer_config.json", "generation_config.json", "chat_template.jinja", "special_tokens_map.json", "vocab.json", "merges.txt"]:
        try:
            ...
            pass
    log(f"written {args.out}: {total/1e9:.2f}B params, {shard_i} shards")
